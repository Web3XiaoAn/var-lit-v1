"""Local WebSocket receiver for the Var-Lit V1 Chrome bridge."""
...
COMMAND_PROTOCOL_VERSION = "var-lit-v1-command-v1"
COMMAND_EXTENSION_BUILD = "var-lit-v1"
CHROME_EXTENSION_ORIGIN_RE = re.compile(r"\Achrome-extension://[a-p]{32}\Z")
...
class VariationalMonitor:
    ...
    _portfolio_received_monotonic: float | None = None
    portfolio_request_id: str | None = None
    portfolio_captured_at: str | None = None
    portfolio_fingerprint: str | None = None
    portfolio_content_revision: int = 0
    _next_heartbeat_check_ts: float = 0.0
    _stale_alert_sent: bool = False
    _last_hourly_alert_hour: int = 0
    _next_trade_event_seq: int = 1
    _quote_revision: int = 0
    _trade_revision: int = 0
    on_quote_update: Callable[[], None] | None = field(default=None, repr=False)
    on_trade_event: Callable[[], None] | None = field(default=None, repr=False)
    ...
    async def process_rest_event(self, payload: dict[str, Any]) -> list[str]:
        ...
        async with self._lock:
            self._update_quote(parsed, captured_at=payload.get("timestamp"))
        ...
        async with self._lock:
            ...
            if stream == WS_EVENTS_PATH:
                for event in self._iter_event_messages(parsed):
                    ...
                    trade_line = self._update_trade_event(event, captured_at=payload.get("timestamp"))
                    if trade_line:
    ...
    def _update_quote(self, payload: Any, captured_at: Any = None) -> None:
        ...
        self._quote_revision += 1
        if self.on_quote_update is not None:
            self.on_quote_update()

    def _update_trade_event(self, payload: Any, captured_at: Any = None) -> str | None:
        if not isinstance(payload, dict):
            return None
        ...
        summary = {
            ...
            "role": data.get("role", "-"),
            "source_rfq": data.get("source_rfq"),
        ...
        if self.on_trade_event is not None:
            self.on_trade_event()
    ...
    def _update_portfolio(
        ...
        *,
        ...
        fingerprint_rows = [
            ...
            for asset, row in sorted(next_positions.items())
        ]
        ...
        if fingerprint != self.portfolio_fingerprint:
            self.portfolio_content_revision += 1

        self.positions = next_positions
        self._portfolio_received_monotonic = asyncio.get_running_loop().time()
        self.portfolio_request_id = str(request_id or "").strip() or None
        self.portfolio_captured_at = str(captured_at or "").strip() or None
        self.portfolio_fingerprint = fingerprint
        self.portfolio_summary = {
            ...
            "request_id": self.portfolio_request_id,
            "captured_at": self.portfolio_captured_at,
            "fingerprint": self.portfolio_fingerprint,
            "content_revision": self.portfolio_content_revision,
    ...
    async def get_trading_state(self) -> dict[str, Any]:
        async with self._lock:
            ...
            return {
                ...
                "portfolio_age": (
                    max(0.0, now_ts - self._portfolio_received_monotonic)
                    if self._portfolio_received_monotonic is not None
                    else None
                ),
                "portfolio_request_id": self.portfolio_request_id,
                "portfolio_captured_at": self.portfolio_captured_at,
                "portfolio_published_at": self.portfolio_summary.get("published_at"),
                "portfolio_fingerprint": self.portfolio_fingerprint,
                "portfolio_content_revision": self.portfolio_content_revision,
            }
    ...
    async def get_update_revisions(self) -> tuple[int, int]:
        """Return quote/trade revisions for event-driven consumers.

        Revisions, rather than a destructive queue, allow the strategy and
        recovery loops to coalesce the same update independently.
        """
        async with self._lock:
            return self._quote_revision, self._trade_revision
...
class CommandBroker:
    ...
    async def attach_extension_transport(self, websocket: websockets.ServerConnection, role: str = "extension-bridge") -> None:
        async with self._lock:
    ...
    async def on_disconnect(self, websocket: websockets.ServerConnection) -> None:
        async with self._lock:
            ...
            if websocket is self._extension:
                ...
                pending_futures = list(self._pending_futures.items())
                self._pending_futures.clear()
                for request_id, future in pending_futures:
                    if not future.done():
                        future.set_result(
                            {
                                "type": "ORDER_RESULT",
        ...
        if msg_type == "PLACE_ORDER":
            await self._send(
                websocket,
                {
                    ...
                    "ok": False,
                    "error": "WebSocket clients cannot submit orders.",
                    "timestamp": utc_now(),
                },
            )
            return
        if msg_type == "ORDER_RESULT":
            await self._handle_order_result(websocket, payload)
            return
    ...
    async def _handle_register(self, websocket: websockets.ServerConnection, payload: dict[str, Any]) -> None:
        ...
        error: str | None = None
        if role != "extension":
            error = "Only the Chrome extension role may register."
        elif protocol_version != COMMAND_PROTOCOL_VERSION:
            error = "Chrome extension command protocol version mismatch."
        elif build != COMMAND_EXTENSION_BUILD:
            error = "Chrome extension build mismatch."

        async with self._lock:
            if (
                error is None
                and self._extension is not None
                and self._extension is not websocket
            ):
                error = "A Chrome extension command client is already registered."
            if error is None:
                self._roles[websocket] = "extension"
        ...
        await self._send(
            websocket,
            {
                "type": "REGISTER_ACK",
                "ok": error is None,
                "role": "extension" if error is None else None,
        ...
        if not self.quiet and error is None:
            print("[COMMAND] registered role=extension", flush=True)
    ...
    async def _handle_order_result(
        ...
        async with self._lock:
            authorized = websocket is self._extension
        if not authorized:
            await self._send(
                websocket,
                {
                    "type": "ERROR",
                    "ok": False,
                    "error": "ORDER_RESULT is accepted only from the registered extension.",
                    "timestamp": utc_now(),
                },
            )
            return
    ...
    async def request_place_order(
        ...
        *,
        ...
        firm_quote: dict[str, Any] | None = None,
        guard: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        side_n = side.strip().upper()
        if side_n not in {"BUY", "SELL"}:
            return {"type": "ORDER_RESULT", "ok": False, "error": "Invalid side. Use BUY or SELL."}
        try:
            amount_value = float(str(amount))
            if not math.isfinite(amount_value) or amount_value <= 0:
                raise ValueError
        except (TypeError, ValueError):
            return {"type": "ORDER_RESULT", "ok": False, "error": "Invalid amount. Must be positive."}
        if base_qty is not None and str(base_qty).strip():
            try:
                base_qty_value = float(str(base_qty))
                if not math.isfinite(base_qty_value) or base_qty_value <= 0:
                    raise ValueError
            except (TypeError, ValueError):
                return {"type": "ORDER_RESULT", "ok": False, "error": "Invalid base_qty. Must be positive."}
        if fetch_stage not in {"quote", "commit"}:
            return {
                "type": "ORDER_RESULT",
                "ok": False,
                "error": "Invalid fetch_stage. Use quote or commit.",
            }
        ...
        payload = {
            ...
            "side": side_n,
            "amount": str(amount),
            "baseQty": str(base_qty) if base_qty is not None else None,
            "market": market,
            "account": account,
        ...
        async with self._lock:
            ...
            if extension is None:
                return {"type": "ORDER_RESULT", "requestId": request_id, "ok": False, "error": "No extension command client connected."}
            extension_role = self._roles.get(extension, "extension")
            self._pending_futures[request_id] = future

        try:
            # Do not hold the broker lock across network I/O.  Disconnect
            # handling can then resolve this future immediately, and caller
            # cancellation cannot leave an unreachable pending request behind.
            if not await self._send(extension, payload):
                return {
                    "type": "ORDER_RESULT",
                    "requestId": request_id,
                    "ok": False,
                    "error": "Extension disconnected while sending order command.",
                }
            timeout = (timeout_ms / 1000) if timeout_ms else 10
            ...
            return {
                ...
                "requestId": request_id,
                "ok": False,
                "error": f"Timed out waiting {timeout:.1f}s for extension order result via {extension_role}.",
            }
        finally:
            async with self._lock:
                if self._pending_futures.get(request_id) is future:
                    self._pending_futures.pop(request_id, None)

    async def _send(self, websocket: websockets.ServerConnection, payload: dict[str, Any]) -> bool:
        try:
...
COMMAND_MESSAGE_TYPES = {"REGISTER", "PING", "PLACE_ORDER", "ORDER_RESULT"}


def valid_chrome_extension_origin(origin: str) -> bool:
    return CHROME_EXTENSION_ORIGIN_RE.fullmatch(str(origin or "")) is not None
...
def is_command_message(raw: str) -> bool:
    ...
    if not isinstance(payload, dict):
        return False
    return str(payload.get("type", "")).upper() in COMMAND_MESSAGE_TYPES
...
async def run_receiver_server(
    ...
    command_bridge: bool = False,
) -> websockets.asyncio.server.Server:
    async def handler(websocket: websockets.ServerConnection) -> None:
        if command_broker is not None and command_bridge:
            await command_broker.attach_extension_transport(websocket)
        try:
            async for message in websocket:
                if isinstance(message, bytes):
                    message = message.decode("utf-8", errors="replace")
                if command_bridge and command_broker is not None and is_command_message(message):
                    await command_broker.handle_raw_message(websocket, message)
                else:
                    await sink.handle(channel, message)
        finally:
            if command_broker is not None and command_bridge:
                await command_broker.on_disconnect(websocket)
...
async def run_command_server(
    ...
    async def handler(websocket: websockets.ServerConnection) -> None:
        origin = str(websocket.request.headers.get("Origin", ""))
        if not valid_chrome_extension_origin(origin):
...
def parse_args() -> ListenerConfig:
    parser = argparse.ArgumentParser(description="Run local receivers for Var-Lit V1 bridge events.")
