...
服务器浏览器采用普通 Google Chrome + Xvfb 虚拟显示器。Chrome、OKX Wallet 与
Variational 的行为和桌面模式一致；不需要长期打开远程桌面。
...
```bash
systemctl status var-lit-v1-display.service var-lit-v1-chrome.service
...
```bash
ssh -L 5900:127.0.0.1:5900 <购买服务器时配置的SSH用户>@服务器IP
```

用本机 VNC 客户端连接 `127.0.0.1:5900`。不要把 5900 端口开放到公网。然后由账户
持有人本人完成：
...
```bash
cd /opt/var-lit-v1
sudo -u varlit venv/bin/python tools/check_host_readiness.py --phase server
sudo -u varlit venv/bin/python tools/probe_extension.py --duration 60
```
...
- 启动：`var-lit-v1-display` → `var-lit-v1-chrome` → `var-lit-v1-runtime`。
